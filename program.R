  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        # 隐含波动率锥标签页
        tabPanel(
          title = div(icon("bar-chart"), "隐含波动率锥"),
          value = "iv_cone",
          div(class = "plot-container",
        ...
        tabPanel(
          ...
          value = "hv_cone",
          div(class = "plot-container",
              plotlyOutput("hv_cone_plot", height = "600px")  # 修改为600px，与前两个一致
          )
        ),
...
server <- function(input, output, session) {
  ...
  data_processed <- eventReactive(input$process, {
    ...
    for(i in seq_along(ids)) {
      id <- ids[i]
      if(id %in% colnames(expire_date)) ID$Expire_Date[i] <- na.omit(expire_date[[id]])[1]
      if(id %in% colnames(exercise_price)) ID$Exercise_Price[i] <- na.omit(exercise_price[[id]])[1]
    }
    ...
    latest_date <- max(close_price$时间, na.rm = TRUE)
    today <- Sys.Date()
    diff_dates <- if(latest_date + 1 > today - 1) close_price$时间[close_price$时间 != today] else seq(latest_date + 1, today - 1, by = "day")
      ...
      for(i in 1:total_rows) {
        ...
        if(current_date %in% diff_dates) {
          date_data <- data.frame(
            ID = ids,
            Volume = as.numeric(volume[i, -1]),
            Close_Price = as.numeric(close_price[i, -1]),
            Target_Close_Price = as.numeric(target_close_price[i, -1]),
            Expire_Date = ID$Expire_Date,
            Exercise_Price = ID$Exercise_Price,
            stringsAsFactors = FALSE
          )
          date_data <- subset(date_data, !((is.na(Volume) | Volume==0) & (is.na(Close_Price) | Close_Price==0)))
          if(nrow(date_data) > 0 && !all(is.na(date_data[, -1]))) update_list[[as.character(current_date)]] <- date_data
        }
    ...
    total_dates <- length(names(update_list))
    withProgress(message = '添加利率数据...', value = 0, {
      for(j in seq_along(names(update_list))) {
        date_str <- names(update_list)[j]
        current_date <- as.Date(date_str)
        rf_row <- rf[rf$时间 == current_date, ]
        update_list[[date_str]]$rf <- if(nrow(rf_row) > 0) rf_row$`无风险利率`[1] else NA
        incProgress(1/total_dates)
      }
    })
    ...
    withProgress(message = '计算Level...', value = 0, {
      for(k in seq_along(names(update_list))) {
        date_str <- names(update_list)[k]
    ...
    update_progress("数据清洗完成！", 10)
    
    return(list(update_list = update_list, dates = sort(as.Date(names(update_list)))))
  })
  ...
  atm_iv_data <- eventReactive(input$process, {
    ...
    update_progress("开始计算隐含波动率...", 11)
    
    update_list <- data_processed()$update_list
    type <- input$type
    ...
    implied_vol_call <- function(C_market, S, K, r, T, sigma_lower=1e-6, sigma_upper=5) {
      if (C_market <= max(S - K*exp(-r*T),0) || C_market > S) return(NA)
      tryCatch({
        uniroot(function(sigma) bs_call_price(S,K,r,T,sigma) - C_market, lower=sigma_lower, upper=sigma_upper)$root
      }, error=function(e) NA)
    }
    
    implied_vol_put <- function(P_market, S, K, r, T, sigma_lower=1e-6, sigma_upper=5) {
      if (P_market <= max(K*exp(-r*T)-S,0) || P_market > S) return(NA)
      tryCatch({
        uniroot(function(sigma) bs_put_price(S,K,r,T,sigma) - P_market, lower=sigma_lower, upper=sigma_upper)$root
      }, error=function(e) NA)
    }
    
    update_progress("筛选平值期权...", 12)
    ...
    # 筛选掉临近到期日的合约
    update_progress("筛选到期日...", 13)
    
    At_the_money <- lapply(seq_along(At_the_money), function(i) {
    ...
    names(At_the_money) <- names(update_list)
    
    update_progress("选择主力合约...", 14)
    ...
    atm_max_vol_df <- bind_rows(atm_max_vol, .id = "Date") %>% select(-diff)
    
    update_progress("计算隐含波动率...", 15)
    
    # 计算 IV
    calculate_iv <- function(df, type = "call") {
      df %>%
        mutate(
          Date = as.Date(Date),
          ...
          IV = mapply(
            function(price, S, K, r, T_val) {
              if(any(is.na(c(price,S,K,r,T_val))) || T_val <=0 || price <=0 || S <=0 || K <=0) return(NA)
              if(type=="call") implied_vol_call(price,S,K,r/100,T_val) else implied_vol_put(price,S,K,r/100,T_val)
            },
    ...
    atm_max_vol_df <- calculate_iv(atm_max_vol_df, type)
    
    update_progress("合并历史IV数据...", 16)
    
    # 合并历史 IV 数据
    hist_data <- historical_data()
    if (!is.null(hist_data) && !is.null(hist_data$atm_max_vol_df_latest)) {
      atm_max_vol_df <- bind_rows(hist_data$atm_max_vol_df_latest, atm_max_vol_df) %>%
        distinct() %>%
        arrange(Date)
    }
  ...
  output$iv_plot <- renderPlotly({
    ...
    update_progress("生成IV趋势图...", 18)
    
    atm_max_vol_df <- atm_iv_data()$atm_max_vol_df
  ...
  output$iv_cone_plot <- renderPlotly({
    ...
    update_progress("生成隐含波动率锥...", 19)
    
    atm_max_vol_df <- atm_iv_data()$atm_max_vol_df
    
    # 计算每个到期日的隐含波动率分布
    iv_cone <- atm_max_vol_df %>%
      group_by(Expire_Date, Date) %>%
      summarise(mean_IV = mean(IV, na.rm = TRUE), .groups = "drop") %>%
    ...
    # 提取最新日期的当前IV（用于红点标记）
    latest_date <- max(atm_max_vol_df$Date, na.rm = TRUE)
    current_iv <- atm_max_vol_df %>%
    ...
    fig <- plot_ly() %>%
      ...
      layout(
        title = list(
          ...
          y = 0.95,
          xanchor = "center",
          font = list(size = 18, family = "Microsoft YaHei", color = "#333")
        ),
        xaxis = list(
  ...
  output$hv_cone_plot <- renderPlotly({
    ...
    update_progress("生成历史波动率锥...", 20)
    
    # 处理完成后隐藏进度条
    Sys.sleep(0.5)  # 短暂延迟让用户看到100%
