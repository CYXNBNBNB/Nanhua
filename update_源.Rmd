...
```{r}
# 合并新旧数据并去重
# 对每个日期，如果有重复，保留Volume最大的那个
result_df <- bind_rows(atm_max_vol_df, atm_max_vol_df_latest) %>%
  distinct() %>%
  group_by(Date) %>%
  slice_max(Volume, n = 1, with_ties = FALSE) %>%  # 在每个日期组内，选择Volume最大的行
  ungroup() %>%
  arrange(desc(Date))
...
fig <- plot_ly() %>%
  ...
  #-------------------------------------------------------
  # 25%-75% 蓝色区间 / Blue band for 25%-75%
  #-------------------------------------------------------
  add_trace(
    data = iv_cone,
    x = ~Expire_Date, y = ~p75,
  ...
  #-------------------------------------------------------
  # 当前IV（红色菱形）/ Current IV (red diamond marker)
  #-------------------------------------------------------
  add_trace(
    data = current_iv,
  ...
  plot_bgcolor = "#fafafa",
  paper_bgcolor = "#ffffff",
  margin = list(t = 120)  # 💡 增加顶部留白，避免标题与图例重叠
)
